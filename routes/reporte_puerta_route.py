...
def reporte_puerta_historico( request ):
    ...
    puerta_existe = Puerta.objects.filter( id_puerta=id_puerta, instalacion__id_instalacion=id_instalacion ).exists()
    ...
    registros = PuertaPrincipal.objects.filter(
        puerta__id_puerta=id_puerta, 
        persona__id_persona__isnull=False
    ).order_by( '-fecha_hora' ) # order_by('-fecha_hora') los ordena del más reciente al más viejo
